...
2.  **创建项目**
    ```bash
    ng new {project-name} --style css --routing --directory frontend
    ```

3.  **集成依赖**
    ```bash
    cd frontend
    npm install primeng@^21 @primeuix/themes primeicons tailwindcss @tailwindcss/postcss postcss tailwindcss-primeui
    ```
...
- **创建环境基础文件**
    ...
    export const environmentBase: Environment = {
      ...
      mock: false, // 默认关闭
      api: {
        gateway: '{local-gateway-url}', // 本地开发网关
      }
    };
    ```
...
- **配置 `angular.json`**
    ...
    "serve": {
      "configurations": {
        "debug": {
          "buildTarget": "{project-name}:build:debug"
        },
        "dev": {
          "buildTarget": "{project-name}:build:dev"
        },
        "test": {
          "buildTarget": "{project-name}:build:test"
        },
        "uat": {
          "buildTarget": "{project-name}:build:uat"
        }
      },
...
- **创建拦截器**
    ...
    -   **`url-format.interceptor.ts`**: 自动拼接网关和微服务路径。
        ...
        /**
        * 定义一个上下文令牌，用于在请求中标记是否传递服务在网关中的名字。
        */
    ...
    -   **`response.interceptor.ts`**: 统一处理成功与失败的响应，并提供全局错误提示。
        ...
        function handleSuccess(res: HttpResponse<unknown>) {
          // 业务层级错误处理，以下是假定 RESTful 有一套统一输出格式（指不管成功与否都有相应的数据格式）情况下进行处理
          // 例如响应内容：
...
有关详细的开发规范、目录结构和编码准则，请参阅项目根目录下的 [`docs/standards/code-standard/frontend-develop.md`](../standards/code-standard/frontend-develop.md) 文件。
...
- **启动本地调试环境** (环境配置默认指向 `{local-gateway-url}` 网关):
  ```bash
  npm start
  ```

- **启动开发环境** (环境配置默认指向 `{dev-api-url}` 网关):
  ```bash
  npm run start:dev
  ```
