...
public record OperationRecordOutputDto
{
    ...
    /// 从框架形态转换。
    /// </summary>
    /// <param name="info">框架记录。</param>
    /// <param name="includeHostOnlyFields">
    /// 读者是否为宿主。<see langword="false"/> 时 <see cref="FailureDetail"/> 与
    /// <see cref="CorrelationId"/> 一律置空——<b>字段级裁剪必须在服务端做</b>，
    /// 交给界面"不显示"只是把数据下发了却假装看不见。
    /// </param>
    public static OperationRecordOutputDto FromInfo(OperationRecordInfo info, bool includeHostOnlyFields) => new()
    {
        Id = info.Id,
        Action = info.Action,
        TargetId = info.TargetId,
        TargetName = info.TargetName,
        AuthorizationBasis = info.AuthorizationBasis,
        Outcome = info.Outcome.ToString(),
        ...
        FailureDetail = includeHostOnlyFields ? info.FailureDetail : null,
        CorrelationId = includeHostOnlyFields ? info.CorrelationId : null,
        ActorIsTarget = info.ActorId is null
            && info.Outcome == OperationRecordOutcome.Succeeded
            && IsSelfActing(info.Action)
    };
