...
@Component({
        ...
        <div class="pt-2 flex gap-2 flex-wrap justify-start">
          @for (primaryColor of primaryColors(); track primaryColor.name) {
            <button
              ...
              [style]="{
                'background-color': primaryColor?.name === 'noir' ? 'var(--p-text-color)' : primaryColor?.palette?.['500']
              }"
            ></button>
          }
        </div>
        ...
        <div class="pt-2 flex gap-2 flex-wrap justify-start">
          @for (surface of surfaces; track surface.name) {
            <button
              ...
              class="border-none w-5 h-5 rounded-full p-0 cursor-pointer outline-none outline-offset-1"
              [style]="{
                'background-color': surface?.name === 'noir' ? 'var(--p-text-color)' : surface?.palette?.['500']
              }"
            ></button>
          }
        </div>
  ...
  host: {
    class:
      'hidden absolute top-12 right-0 w-72 p-4 bg-surface-0 dark:bg-surface-900 border border-surface rounded-border origin-top shadow-[0px_3px_5px_rgba(0,0,0,0.02),0px_0px_2px_rgba(0,0,0,0.05),0px_1px_4px_rgba(0,0,0,0.08)]'
  }
})
export class ThemeConfigurator implements OnInit {
  ...
  // Menu Mode is removed as it's not requested and might conflict with gemini layout logic
