...
| 审查结果 | 下一步 | 说明 |
|---------|--------|------|
| ✅ 无 P0 违规 | 写入审查报告 | 在输出中标记“具备测试验证条件” |
| ❌ 有 P0 违规 | 阻塞，必须修复后重新审查 | 修复后再次执行 code-review |
| ⚠️ 仅有 P1/P2 | 写入审查报告并记录问题 | P1/P2 不阻塞，但应尽快修复 |

**审查作为 coding Skill 的内置步骤时**：只作为开发阶段自查，不替代独立 `code-review` 阶段。

**审查作为独立阶段时**：完成后输出审查结论和后续上下文；无 P0 标记具备测试验证条件，有 P0 标记需要开发修复。
