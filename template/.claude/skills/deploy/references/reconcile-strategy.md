...
1. `docs/standards/agent-workflow.md`（Phase 6 门禁、确认点）
2. `docs/deploy/`（环境、服务清单、发布/回滚）
3. 根目录 `docker-compose.yml` / `compose.yml` / `Dockerfile` / CI-CD / `.env.deploy`

**冲突示例**：`docs/deploy/` 声明单容器，实际项目已是 compose 多服务 → 提示差异，建议更新 `docs/deploy/`，本次以实际编排为准并请用户确认。

**缺失降级**：缺部署配置时**只输出部署方案，不猜测生产服务器/端口/密钥/回滚方案**（参考 `templates/server-config-template.md`、`templates/docker-compose-template.yml`），提示可创建。

### 🚫 deploy 红线（高于通用降级）

- 未确认目标环境/回滚方案时不部署。
- 不把密码/Token/私钥/SSH 私钥写入仓库。
- 不因缺配置而自行猜测生产服务器。
