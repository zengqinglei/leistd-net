# Vitest 配置参考样例（仅 Angular/Vite 栈）

> **定位**：这是 **Angular + Vitest 测试配置的参考样例，不是通用默认、不自动套用**。仅当项目确为该栈、且用户要求生成测试配置时参考；其它技术栈请按同类思路生成对应配置。
> **其中的覆盖率门槛等数值不预设**——门槛须按项目约定（见 `coverage-thresholds-template.md` 的提问清单），本样例里的数值仅为占位示意，套用前必须替换为项目已约定的值。
