...
启动开发前按顺序检测，命中缺失或冲突时按核心原则处置：

1. `docs/standards/tech-stack.md`（技术栈声明）
2. `docs/standards/agent-workflow.md`（路径、状态机）
3. `docs/standards/code-standard/backend-develop.md`
4. `docs/standards/code-standard/frontend-develop.md`
5. 项目配置与邻近代码（`package.json` / `*.csproj` / `pyproject.toml` / `go.mod`）

**冲突示例**：项目实际用 Vue 但 `tech-stack.md` 声明 Angular → 提示差异，建议更新 `tech-stack.md`，本次按实际代码风格执行。

**缺失降级**：无对应规范时，读现有配置与邻近代码推断最小规则，不套用模板默认 .NET/Angular 作为事实。
