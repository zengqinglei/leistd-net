...
用 `YYYY-MM-DD-<topic>.md` 命名并相互链接。**这三个目录只放在途工作**：诊断完就删评估、做完就删计划，
长期有效的结论必须先沉淀到 `architecture/` 或 `framework/` / `template/`。Git 是历史的唯一归档，
不在 `docs/` 里维护第二份。目录为空时不存在，不预建空目录或无内容的分类。
