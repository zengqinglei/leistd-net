...
> §6.4 的按租户分库能力是**第三项框架改造**，但它不阻塞开工：不分库模式今天就能跑，且业务代码对两种模式无感（§6.2）。何时补取决于第一个需要独享库的租户何时出现。

### 5.2 迁移形态：由条件剪裁二选一（强烈建议同批）

**现状**：模板在 `app.Run()` 之前无条件执行——有迁移走 `MigrateAsync`、无迁移走 `EnsureCreatedAsync`，**没有任何门禁**。生产每个 Pod 启动都尝试改 schema，多副本同时启动即并发迁移。

**不用运行时开关，用模板参数** `IncludeDbMigrator`（默认 `false`）。条件剪裁让能力**根本不在产物里**，而运行时开关只是把能力关着——总有人能改配置打开。这也与 `IncludeXxx` 的既有机制一致。

| 模式 | 参数 | 启动时行为 | 适用 |
| --- | --- | --- | --- |
| **默认** | `false` | 保持现状：有迁移则 `Migrate`，无迁移则 `EnsureCreated` | 开源体验、本地、小型单实例部署 |
| **迁移器** | `true` | **只校验、绝不施加**：有待执行迁移则启动失败并打印清单 | 大型项目生产部署 |

两种模式各自**只有一个行为**，不存在环境分支：模式 1 施加，模式 2 校验。

#### 为什么模式 2 仍要在启动时校验

"启动什么都不做"会把一个 DDL 隐患换成另一个隐患：**带着旧 schema 静默跑起来**。故障会以离原因很远的运行期错误冒出来（缺列、类型不符），比启动失败难查得多。

这次校验是只读的一次查询，不做 DDL、不需要 DDL 权限、不构成"影响启动"，但它把"忘了跑 Job 就发版"这个最常见的操作失误挡在门外。
...
| 命令 | 行为 |
| --- | --- |
| 无参 | **只读预演**：打印待执行迁移清单与生成的 SQL，不改任何东西 |
| `--apply` | 施加迁移 |

这正好落实既定的生产变更流程：先只读预演给人确认，再执行写入。运行镜像因此不需要 EF 工具链；对方文档已把 `DbMigrator` 列入"服务内工作负载"，形态一致。

**模式 2 下本地开发也跑迁移器**（`dotnet run --project ...DbMigrator -- --apply`）。多一条命令，换来本地与生产完全同构——模式 2 面向本来就有迁移流程的团队，同构比省一条命令值。

#### 生产应用账号不给 DDL 权限

这条把"应用不改 schema"从约定变成机制。模式 2 的产物里没有施加迁移的代码，权限再收掉一层，两道一起才闭合。

#### 不做的三件事

| 不做 | 理由 |
| --- | --- |
| 迁移加分布式锁 | 多副本并发迁移只出现在模式 1，而模式 1 的定位就是单实例/本地。需要多副本就该选模式 2 |
| 把种子搬进迁移器 | 种子幂等、与 schema 无关，本次只动 schema 边界 |
| 第三种 CLI 模式 | 预演 + 施加够用，CI 直接把 `--apply` 作为 Job |

#### 与分库模式的联动约束

**按租户分库必须配迁移器模式。** 分库后迁移要遍历默认库 + 每个租户库、逐库报告结果、部分失败要能定位——让服务启动时干这件事是最脆弱的形态。因此"租户连接串"与 `IncludeDbMigrator=false` 是不该组合的参数，模板层面应当直接拒绝该组合。

#### 零停机

expand/contract：加可空列 → 回填 → 部署兼容新旧的代码 → 切换 → 下个版本删列。**任何"一步重命名"都会在滚动发布期间打断正在运行的旧副本。**
...
| 模式 | 形态 | 隔离手段 |
| --- | --- | --- |
| **不分库**（默认） | 一个默认库，**每个服务一个独立 schema**，租户共表 | `TenantId` 列 + 全局查询过滤器 |
| **分库**（按租户可选） | 该租户独立一个库，**所有服务共用这个租户的连接串**，各服务仍在自己的 schema 里 | 物理库隔离 + `TenantId` 列 + 过滤器（双重） |

分库是**按租户逐个可选**的：租户配了连接串就走它的库，没配就落默认库。同一套部署可以两种租户并存。

### 6.2 贯穿两种模式的不变量

**`TenantId` 列与全局过滤器在两种模式下都保留，一个都不能撤。** 三个理由：

1. 分库模式下一条路由错的连接会泄漏整个库，过滤器是第二道防线。"分库了就不用过滤器"是灾难性优化。
2. **业务代码对两种模式无感**——同样的实体、同样的查询、一行 `WHERE` 都不写。这正是支持两种模式在业务层几乎零成本的原因。
3. 租户从默认库迁到独享库（或反向）时，数据形状不变，只搬数据不改代码。

### 6.3 每个服务独立 schema

框架组件的表名**没有硬编码 schema**，因此各服务在自己的 `ConfigureModel` 里设默认 schema 即可覆盖全部实体（含框架的租户注册表、权限授予表）：

```csharp
protected override void ConfigureModel(ModelBuilder modelBuilder)
{
    modelBuilder.HasDefaultSchema("crm");       // [T] 每个服务一个，务必与服务名一致
    modelBuilder.ConfigureBaseEntities();
    // ...
}
```

**同时必须给每个服务独立的迁移历史表**，否则多服务共库时会争用同一张 `__EFMigrationsHistory`：

```csharp
options.UseNpgsql(connectionString, npgsql =>
    npgsql.MigrationsHistoryTable("__EFMigrationsHistory", "crm"));   // [T]
```

这两行都是模板调整（`[T]`），不需要改框架。**漏掉第二行的后果最隐蔽**：两个服务的迁移记录混在一张表里，各自的迁移器会互相看到对方的记录。

### 6.4 分库模式需要补的框架能力

现状：`TenantConfiguration` 是封闭六字段，**没有连接串**；`EfCoreTenantStore<TDbContext>` 从业务 DbContext 读 `TenantRecord`。要支持分库需要补：

**1. 租户连接串**：`TenantRecord` / `TenantConfiguration` 增连接串字段（空 = 用默认库），管理端可维护。

**2. 连接串解析进 DbContext 创建**。模板已用 `(sp, options)` 重载注册 DbContext，接缝就在那里：

```csharp
services.AddDbContext<CrmDbContext>((sp, options) =>
{
    var resolver = sp.GetRequiredService<ITenantConnectionStringResolver>();
    options.UseNpgsql(resolver.Resolve(), npgsql => npgsql.MigrationsHistoryTable(...));
});
```

连接串不属于影响模型的选项，EF 的模型缓存仍可跨租户共享，不会按租户重复编译模型。
...
**5. 跨租户聚合的显式出口**：分库后一条 SQL 不可能跨租户聚合。SaaS Control Plane 的跨租户报表需要按库循环 + 应用层汇总，或另建只读汇总库。**这条能力在不分库模式下"免费"，分库后必须显式实现**——是分库最大的隐性成本。
...
其余租户留在默认库。**不要因为"将来可能需要"就给所有租户分库**——每个独享库都要单独迁移、单独备份、单独监控，而收益只对上述三类租户成立。

**否决共库分租户 schema**（即同一个库里按租户建 schema）：EF Core 支持别扭，备份粒度并没有变好，等于承担了迁移复杂度却没拿到物理隔离。schema 只用来分**服务**，不用来分租户。
...
| 隔离模式 | 同一套业务用例在**不分库**与**分库**两种模式下各跑一遍，结果必须一致 | 业务代码对模式无感（§6.2）是设计前提，用例跑两遍就是它的证明 |
...
- [ ] 迁移器模式下，存在待执行迁移时服务**启动失败并打印清单**；服务进程从不施加迁移。
- [ ] 每个服务目录可独立 `git init` 成仓并单独构建，不依赖根目录下的任何共享工程。
- [ ] 多服务共库时各服务在独立 schema、各有独立的 `__EFMigrationsHistory`，迁移器互不干扰。
- [ ] 分库模式下迁移器**逐库报告结果**；某个租户库失败不被总结果掩盖。
