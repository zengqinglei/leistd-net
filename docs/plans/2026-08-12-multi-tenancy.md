...
└── Leistd.MultiTenancy.Tests/                     # 单元 + Sqlite 过滤器 + TestServer 中间件端到端
    ...
    ├── TenantStoreManagerTests.cs                 # 归一化、唯一索引与并发冲突、启停/删除即时生效
...
14. **第八轮审查采纳的修复**：清掉三处把"当前事实"讲错的缓存表述（`ITenantManager` 的类型注释与 `FindAsync` 注释、集成测试里的停用注释）。
...
15. **第九轮审查采纳的修复**：组合根 `AddMultiTenancyEfCore` 的注释仍写"依赖下方注册的分布式缓存"，已改为"存储直接读库，除 DbContext 外无基础设施依赖"。
...
18. **第十二轮审查采纳的修复**（上一轮机制的两个直接分支都会漏清租户）：
    - **CORS 分离模式下前端读不到标记头。** `AllowAnyHeader()` 只放行**请求**头；自定义**响应**头不在 CORS 安全清单里，不 `WithExposedHeaders` 则浏览器不交给 JS，`error.headers.get()` 恒为 null。而 `frontend/README.md` 明确把"模式二：CORS 分离访问"列为支持的部署形态——也就是说上一轮交付的机制在两种官方模式之一里**根本不生效**。两侧测试都发现不了：后端直读 TestServer 原始响应头，前端手工构造 `HttpHeaders`，谁也没经过浏览器的 CORS 过滤。修法是显式暴露该头，并补一条带 `Origin` 的集成测试同时断言 `X-Tenant-Invalid` 与 `Access-Control-Expose-Headers`（测试需自行开 `Cors:AllowAnyLocalhost`——默认不放行任何来源时 CORS 中间件什么都不写，断言无从谈起）。
      - 也考虑过改用响应体错误码（跨域天然可读、复用现有 `ApplicationHttpError.code` 管线，且这正是最初文档设想的形态），但那是把上一轮刚落地的机制重做一遍；暴露响应头是一行配置且已被测试钉死，同时"会话层信号走响应头、业务错误走 body"的分层也更干净。
    - **`SILENT_AUTH` 连带跳过了必须做的租户清理。** 清租户原先嵌在 `!SILENT_AUTH` 分支里，而 `/auth/me` 与 `/permissions/current` 都是静默请求：租户在启动校验之后、这两个请求之前被停用时，标记 401 照常返回，租户却留了下来。职责拆开——**任意** 401 带标记就清租户（那是"这个租户已经没了"的事实），只有非静默 401 才清认证态并跳转（静默的语义只是"别为后台请求打断用户"）。补交叉用例：带标记的静默 401 清租户但不碰认证与路由。
    - 两条都做了反向验证（撤掉 `WithExposedHeaders` / 把清租户放回非静默分支，对应用例立即变红）。
    - **顺带沉淀一条模板编写规则**（同一个坑两轮各踩一次，每次要一整轮矩阵才暴露）：前端条件块的空行必须放在块**内部**——紧跟 `//#if` 写内容、空行留在 `//#endif` 之前。放在标记外侧时，开启与关闭两种形态必有一种产生双空行或块首空行，prettier 判 lint 失败，而只跑其中一个场景发现不了。已写入 `docs/template/development-guide.md` §3，并把三个租户场景补进那里的标准场景矩阵表（当初加了脚本场景却没记文档，是我的遗漏）。
