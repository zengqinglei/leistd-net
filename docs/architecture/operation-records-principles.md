...
**`Host` 同时决定记录所在的层。** 宿主读的是宿主层（`TenantId` 为空），租户层里一条 `Host` 记录谁都看不见。因此：租户上下文里被拒的宿主动作（租户用户调用宿主接口——宿主最该看到的安全事件）写进宿主层，来源租户另存 `ActorTenantId`；租户上下文里**成功**的宿主动作直接报错，因为成功记录必须与租户库里的业务事务同生共死，而它又不能留在租户层——这类动作的可见性判错了，或者该切到宿主上下文再记。
...
**查询的可见范围没有默认值。** 与动作定义的可见性必填同理：可见性是安全边界，任何默认值都是替调用方做的决定——默认不过滤，漏传一个参数就让租户管理员看到别人的本人级记录；默认最严，宿主又会查出空结果。所以「谁在读」由调用方显式给出，不过滤也必须显式写出来。
...
动作码经注册式定义登记类别、可见性与严重度。**写入未登记的动作码是编码错误，当场抛出**：可见性只能来自定义，任何兜底都是替写入方做的决定——默认给租户看是用「宽容」换「泄露」，默认按 `Host` 落库又会让租户上下文里的记录谁都看不见（见第 4 节）。它与参数漏传同类，确定性地每次发生，第一次跑到就会响。读取时遇到已不再登记的历史码则降级显示裸码，审计记录是既成事实。

同理，筛选选项按当前读者的可见性裁剪——否则筛选框里摆着一堆他永远筛不出东西的项，而「筛了没有」与「看不到」在界面上长得一样。

### 5.1 失败记录独立提交

成功记录与它描述的变更同生共死；失败记录描述的是一次没发生的变更，没有可以同生共死的对象，而业务拒绝之后几乎总是紧跟着回滚。因此失败记录在独立的事务里写入并提交，写入也不接受取消——被审计的一方断开连接，不能让审计作废。代价是第二个事务：它只插一行，常规数据库上不与外层冲突；只允许单个写事务的数据库（SQLite）在外层已有未提交写入时会等锁超时，记录丢失并留下错误日志。
...
**分库租户是同一个陷阱的下一层**：`IgnoreQueryFilters()` 只能放开同一个库里的租户，独立库租户的记录在它自己的库里，只连宿主库的作业同样静默漏掉它们。后台扫描要按物理库逐个执行（框架的 `ITenantDatabaseEnumerator` 列出宿主库与每个独立库），每个库单独隔离失败。
...
判据是「谁、何时、做了什么、结果如何」再加一条「凭什么」：凡是能改变**某人能做什么、某数据是什么、某人是谁**的操作都必须记，**被拒与失败同样要记**——「谁在反复尝试登录失败」正是审计最该回答的问题。高频的顾虑用容量策略解决（同一主体的连续失败按阈值采样、次数随记录落库），不靠不记。反过来，这三样都不改变的高频写入（个人的时区、语言偏好）不记：审计表的价值来自密度。

## 8. 刻意不做的能力

- **变更明细（字段级 diff）不在本组件范围。** 那是实体变更追踪的量级：一张明细表加一套追踪拦截，塞进操作记录只会得到半个审计日志。契约上也不留「是否带变更明细」这类没有能力支撑的标记——看起来支持、实际什么都不记，比明确不支持更误导。真有需求时以独立组件完整落地，且**可审计字段必须 allowlist 而非 denylist**：自动比对全部属性必然把密码哈希、安全戳、令牌写进审计表，denylist 一次漏配就是凭据落库。
- **不做哈希链。** 链的完整性来自写入时算出并落库的哈希，不来自列是否存在，预留空列没有收益；在写入路径上算链要先读上一条，把追加写串行化。需要防篡改时先在数据库层收敛权限（见部署说明），链若启用，从启用之时算起。
- **索引按证据加。** 这是写多读少的追加表，每个索引的代价摊到每一次写入上；在出现真实慢查询之前，不为猜测的查询模式加索引。
