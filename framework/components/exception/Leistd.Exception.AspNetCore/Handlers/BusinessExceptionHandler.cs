    ...
    IServiceProvider serviceProvider) : IExceptionHandler
{
    ...
    /// <summary>
    /// 把结构化字段错误产出为 <c>errors</c> 数组（RFC 9457 Problem Details 的 Leistd 自定义扩展）：每项一个对象，
    ...
    private ProblemDetails BuildProblemDetails(HttpContext httpContext, BusinessException bizException)
    {
        ...
        // 验证异常：errors 为 RFC 9457 Problem Details 的 Leistd 自定义扩展（每项 detail+field+可选 code/localizationKey），
        // 而非 ASP.NET 内置 ValidationProblemDetails 的 {field:[string]} 字典——单一数据源、可承载机器码。
        // 因定义了额外语义，type 用稳定非解析 URI（与自动 400 共用），而非 about:blank。
        ...
        // 一般业务错误：仍固定输出稳定 code 机器契约，超出「仅 HTTP 状态码语义」，据 RFC 9457 §4.2.1 不用 about:blank，
        // 统一给出定义过的 business-error type（title 用状态短语）。
