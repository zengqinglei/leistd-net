...
参考 [Volo.ABP](https://abp.io) 的工程化模式：单点版本、中央包管理（CPM）、Source Link + snupkg 源码调试。
...
# 打包（输出 framework/artifacts/，每个项目产出 .nupkg + .snupkg）
...
所有包随附 `Microsoft.SourceLink.GitHub` 与独立 `.snupkg` 符号包。当仓库已配置 git 远程并推送后，消费方在调试时可直接步进框架源码（VS/Rider 需开启「启用源链接支持」并允许下载符号）。

> 本地无 git 远程时构建会出现 “源链接为空” 警告，属正常现象，不影响功能。
