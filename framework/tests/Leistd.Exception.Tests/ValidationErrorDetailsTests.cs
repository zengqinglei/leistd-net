...
/// <summary>
/// 422 字段错误契约：errors 采用 RFC 9457 §3 validation-error 示例风格的「数组 of 对象」——每项一个
...
public class ValidationErrorDetailsTests
{
    ...
    {
        // 中文资源命中展示键：detail 应为本地化文案；code/field/localizationKey 一并在同一项对象里。
    ...
    {
        // ProblemDetails 默认以 camelCase 序列化：无需 JsonPropertyName 即得 detail/field/code/localizationKey；
        // 可空的 code/localizationKey 未设置时应省略（WhenWritingNull）。
        var options = new JsonSerializerOptions { PropertyNamingPolicy = JsonNamingPolicy.CamelCase };

        // 断言 key 命名（camelCase）——用 ASCII 值避免非 ASCII 转义干扰对 key 的判断。
        var withCode = JsonSerializer.Serialize(
            new ErrorItem("phone taken", "phone", "User:PhoneConflict", "User:PhoneAlreadyUsed"), options);
        Assert.Contains("\"detail\":", withCode);
        Assert.Contains("\"field\":", withCode);
        Assert.Contains("\"code\":\"User:PhoneConflict\"", withCode);
        Assert.Contains("\"localizationKey\":\"User:PhoneAlreadyUsed\"", withCode);
        // 不应出现 PascalCase 键（即 JsonPropertyName 冗余、camelCase 默认已生效）
        Assert.DoesNotContain("\"Detail\":", withCode);
        Assert.DoesNotContain("\"LocalizationKey\":", withCode);

        // 可空机器字段未设置时省略
        var noCode = JsonSerializer.Serialize(new ErrorItem("required", "email", null, null), options);
        Assert.DoesNotContain("\"code\":", noCode);
        Assert.DoesNotContain("\"localizationKey\":", noCode);
