...
public class LoggingHandlerTests
{
    ...
    {
        var (invoker, logger) = Create(new CaptureHandler());
    ...
    {
        var (invoker, logger) = Create(new ThrowingHandler(_ => new HttpRequestException("connection refused")));
    ...
    public async Task 调用方主动取消_不包装原样上抛()
    {
        ...
        var (invoker, _) = Create(new ThrowingHandler(ct => new TaskCanceledException("canceled", null, ct)));
